...
public class ItemShard extends Item {
	...
	public void addInformation(ItemStack is, EntityPlayer par2EntityPlayer, List list, boolean par4) {
		...
		if (!ItemShard.getType(is).isEmpty()) {
			if (ItemShard.getSpecial(is)) {
				if (ItemShard.getType(is).equals(EntityList.classToStringMapping.get(EntityZombie.class))
						&& ItemShard.getSpecial(is)) {
					list.add("Zombie Villager");
				} else if (ItemShard.getType(is).equals(EntityList.classToStringMapping.get(EntitySkeleton.class))
						&& ItemShard.getSpecial(is)) {
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World w, int x, int y, int z, int face, float par8, float par9, float par10) {
		if (!w.isRemote) {
			 ...
			 * }
			 */
			if (w.getBlockId(x, y, z) == SoulShards.Blocks.blockCage.blockID
					&& ItemShard.getType(is) != null) {
				if (ItemShard.getType(is).isEmpty()) return false;
				Integer charges = getCharge(is);
				Integer tier = (int)((Math.log(charges) / Math.log(2))) - 5;
				...
				if ((tier < 4 && canSpawnhere(w.provider.dimensionId, is))
					...
					te.signal = w.isBlockIndirectlyGettingPowered(x, y, z);
					te.setMobType(ItemShard.getType(is), ItemShard.getSpecial(is));
					te.setTier(tier);
				...
				if (!m.isEmpty() && m.equals(ItemShard.getType(is))) {
					Integer i = getCharge(is) + SoulShards.Config.nextTier;
					setCharge(is, (i >= 1024)? 1024 : i);
					w.setBlockToAir(x, y, z);
					// w.setBlock(x, y, z, 0);
					w.playAuxSFX(2001, x, y, z, Block.mobSpawner.blockID
	...
	public static void setType(ItemStack is, String t, String n, boolean bSpecialMob) {
	...
	public static Class getMobClass(ItemStack is) {
		NBTTagCompound tag = initTags(is);
		return EntityList.stringToClassMapping.get(tag.getString("mobtype")).getClass();
	}
	...
	public boolean canSpawnhere(int d, ItemStack is) {
		...
		if (!homeDims.containsKey(getType(is))) return false;
		if (d != -1
				&& type.equals(EntityList.classToStringMapping.get(EntitySkeleton.class))
				&& getSpecial(is)) return false;
		if (d == -1
				&& type.equals(EntityList.classToStringMapping.get(EntitySkeleton.class))
				&& getSpecial(is)) return true;
		if (d == -1 || d == 1) return homeDims.get(type) == d;
		else return homeDims.get(type) == 0;
	}
