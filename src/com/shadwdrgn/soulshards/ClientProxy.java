...
public class ClientProxy extends CommonProxy {
	...
	public void playerInteract(PlayerInteractEvent e) {
		...
		if (e.entityPlayer.worldObj.getBlockId(e.x, e.y, e.z) != Block.glowStone.blockID) return;
		if (!isSoulRune(e.entityPlayer.worldObj, e.x, e.y, e.z)) return;
		if (e.entityPlayer.getHeldItem() == null) return;
		if (e.entityPlayer.getHeldItem().itemID != Item.diamond.itemID) return;
	...
	private boolean isSoulRune(World w, int x, int y, int z) {
		if (w.getBlockId(x + 2, y, z) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x - 2, y, z) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x, y, z + 2) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x, y, z - 2) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x + 1, y, z + 1) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x + 1, y, z - 1) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x - 1, y, z + 1) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x - 1, y, z - 1) != Block.whiteStone.blockID) return false;
		if (w.getBlockId(x + 1, y, z) != Block.netherrack.blockID) return false;
		if (w.getBlockId(x - 1, y, z) != Block.netherrack.blockID) return false;
		if (w.getBlockId(x, y, z + 1) != Block.netherrack.blockID) return false;
		if (w.getBlockId(x, y, z - 1) != Block.netherrack.blockID) return false;
		return true;
	}
