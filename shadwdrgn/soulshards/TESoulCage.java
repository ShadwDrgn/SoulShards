...
public class TESoulCage extends TileEntity implements IInventory {
	...
	private static Field presistenceRequired = EntityLiving.class.getDeclaredFields()[15];
	...
	public void updateEntity() {
		...
		if (count >= delay) {
			...
			for (int i = 0; i < mobCount; ++i) {
				...
				EntityLiving mob = createMobFromName(mobType);
				...
				int numMobs = worldObj.getEntitiesWithinAABB(mob.getClass(), AxisAlignedBB.getAABBPool().getAABB(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1).expand(8.0D, 4.0D, 8.0D)).size();
				...
				double x = xCoord + (rand.nextDouble() - rand.nextDouble()) * 4.0D;
				double y = yCoord + rand.nextInt(3) - 1;
				double z = zCoord + (rand.nextDouble() - rand.nextDouble()) * 4.0D;
				
				mob.setLocationAndAngles(x, y, z, worldObj.rand.nextFloat() * 360.0F, 0.0F);
				if (mob.getCanSpawnHere() || ((tier == 5) && this.getCanSpawnHere(mob))) {
	...
	private EntityLiving createMobFromName(String mobType2) {
		...
		Entity mob = EntityList.createEntityByName(mobType, worldObj);
